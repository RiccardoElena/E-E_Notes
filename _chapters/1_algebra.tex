...
\begin{proof}
  ...
  Si ha che \(\bar{f'} \circ \bar{f}: M \to M\) è un morfismo di monoidi che estende l'identità di \(X\), ovvero:
  \[\forall x \in X, \bar{f'} (\bar{f}(x)) = \bar{f'}(g(x))= g^{-1}(g(x))= x\]
  Considerando la proprietà universale usando \(M\) per entrami i monoidi coinvolti, si ha che anche in questo caso deve esistere un unico morfismo di monoidi che estende l'identità di \(X\). Essendo che anche \(id_M\) è un morfismo di monoidi che estende l'identità di \(X\), si ha che \(\bar{f'} \circ \bar{f} = id_M\).
  Dalla definizione di identità abbiamo che \(\bar{f} = \bar{f'}^{-1} \), ovvero che \(\bar{f}\) è un isomorfismo tra \(M\) e \(M'\). 
\end{proof}
...
\begin{note}
  È importate notare che il morfismo di lunghezza è ben definito, poiché ogni elemento di \(M\) si fattorizza in modo univoco come prodotto di elementi di \(X\).
