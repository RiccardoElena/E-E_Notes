...
Tale notazione permette di formulare in modo più compatto la chiusura di un sottoinsieme \(Y\) di un semigruppo.
  \[(\forall a,b \in Y, a \cdot_S b \in Y)\iff (Y^2 \subseteq Y).\]
...
\begin{definition}{Sottostruttura generata}
  ...
  \[Y^* = \set{1_M} \cup Y^+ = \bigcup_{n=0}^{\infty} Y^n.\]
\end{definition}
...
\begin{example}[]{}
  Esempi di basi (e dunque di monoidi liberi) sono:
  \begin{itemize}
    \item \(A\) è una base di \(A^*\) per ogni alfabeto \(A\).
    \item \(\set{1}\) è una base di \((\N, +, 0)\).
  \end{itemize}
  Contrariamente a quello che si potrebbe pensare, \((\N, \cdot, 1)\) \textbf{non} è un monoide libero con base l'insieme dei numeri primi, poiché \(6 = 2 \cdot 3 = 3 \cdot 2\) ammette due fattorizzazioni distinte.
  Il concetto di struttura libera però si generalizza a diverse strutture algebriche, ad esempio \((\N, \cdot, 1)\) è un \emph{monoide \textbf{commutativo} libero} di base l'insieme dei numeri primi.
\end{example}
...
\begin{theorem}{Proprietà universale dei monoidi (semigruppi) liberi}
  Sia \(M\) un monoide con \(X \subseteq M\). Allora \(M\) è libero di base \(X\) se e solo se, per ogni monoide \(M'\) e ogni applicazione \(f: X \to M'\), esiste un unico morfismo di monoidi \(\bar{f}: M \to M'\) tale che \(\bar{f}_{|_X} = f\), ovvero tale che il seguente diagramma commuta:
  \begin{center}
...
\begin{observation}{}
  È importante notare che il morfismo di lunghezza è ben definito per ogni valore di \(M\), poiché ogni elemento di \(M\) si fattorizza in modo univoco come prodotto di elementi di \(X\).
...
\begin{lemma}[namedlabel={lem:levi}{Levi}]{Levi}
...
\begin{definition}[namedlabel={def:quotient_sets}{Insiemi quoziente}]{Insiemi quoziente}
...
Utilizzando tali insiemi è possibile formulare un importante teorema sui sottomonoidi liberi.

\begin{theorem}[namedlabel={thm:schützenberger_monoids}{Schützenberger sui sottomonoidi liberi}]{Schützenberger sui sottomonoidi liberi}
...
\begin{proof}[\extrasymbol{} Dimostrazione \Namedref{thm:schützenberger_monoids}]
  ...
  \begin{description}
    ...
    \item[\q{\(\impliedby\)}]
      ...
      Per il \Namedref{lem:levi} esiste \(v \in M\) tale che \(x_1' = x_1 v\) e \(x_2 \ldots x_i = v x_2' \ldots x_j'\).
      ...
      Dunque \(X\) è una base di \(N\), e \(N\) è libero.
  \end{description}
...
\begin{definition}{Sottomonoide unitario}
...
Da tale definizione e dal \Namedref{thm:schützenberger_monoids} segue che, dato \(M\) monoide libero, se  \(N \leq M\) è unitario, allora è libero
