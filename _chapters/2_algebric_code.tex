...
\begin{definition}[namedlabel={def:prefix_suffix}{Prefisso e Suffisso}]{Prefisso e Suffisso}
  Dato \(A\) alfabeto finito, diremo che \(\emptyset \neq X \subseteq A^*\) è
  \begin{description}
...
\begin{definition}[namedlabel={def:remainder}{Insiemi resto}]{Insiemi resto}
...
\begin{theorem}[namedlabel={thm:sardinas-patterson}{Sardinas-Patterson}]{Sardinas-Patterson}
...
\begin{proof}
  ...
  \begin{description}
    \item[\q{\(\implies\)}] 
      ...
      Per ipotesi induttiva, esistono \(k,l \geq 1, x_1\ldots x_k,x_1'\ldots x_l' \in X\) tali che:
      \begin{itemize}
        \item \(k+l=n\)
        \item \(x_1\ldots x_k r_{n-1} = x_1'\ldots x_l'\)
        \item \(x_1 \neq x_1'\)
        \item \(\abs{r_{n-1}} \leq \abs{x_l'}\)
      \end{itemize}
      Ora, se \(xw = r_{n-1}\), si ha che
      \[x_1\ldots x_k xw= x_1\ldots x_k r_{n-1} = x_1'\ldots x_l'\]
      Scegliendo dunque \(i = k+1,j=l\), e \(x_i = x_{k+1} = x\) si ha che tutte le condizioni sono soddisfatte.
      Le prime tre condizioni sono ovvie, e l'ultima è data da \(\abs{w}\leq\abs{r_{n-1}}\leq\abs{x_l'}\).
      Se invece \(r_{n-1}w = x\),  concatenando \(w\) a destra di entrambi i membri otteniamo che
      \[x_1\ldots x_k r_{n-1} w = x_1'\ldots x_l' w\]
      Ma sapendo che \(r_{n-1}w = x\), possiamo riscrivere gli ultimi due fattori del primo membro come \(x\), ottenendo
      \[x_1\ldots x_k x = x_1'\ldots x_l' w\]
      In questo caso, scegliendo \(i = l,j=k+1\) e \(x_j' = x_{k+1}' = x\) otteniamo nuovamente tutte le condizioni.
    \item[\q{\(\impliedby\)}]
      ...
      \begin{enumerate}
        ...
        \item \(\abs{x_{i}w} > \abs{x_j'}\):
          ...
          Sappiamo infatti per ipotesi dell'implicazione che \(\abs{w} \leq \abs{x_j'}\) e che \(x_1\ldots x_i w = x_1'\ldots x_j'\). Usando il \Namedref{lem:levi}, ponendo come \(m_1 = x_1\ldots x_i\), \(m_2 = w\), \(m_3 = x_1'\ldots x_{j-1}'\) e \(m_4 = x_j'\), otteniamo che esiste \(v \in A^*\) tale che \(vw=x_j'\).
          ...
          \begin{enumerate}
            ...
            \item \(i + (j-1) = n\)
            \item \(x_1 \neq x_1'\)
            \item \(\abs{v} \leq \abs{x_i}\), poiché da \(vw = x_j'\) e \(\abs{x_{i}w} > \abs{x_j'}\) si ha:
            \[\abs{x_{i}w}  > \abs{vw} \implies \abs{x_{i}} > \abs{v}\]
        \end{enumerate}
...
\begin{proof}[Dimostrazione \Namedref{thm:sardinas-patterson}]
  ...
  \begin{enumerate}
    ...
    \item \(x_1 \neq x_1'\) per scelta
    \item \(\abs{w} = \abs{x_h} \leq \abs{x_k'}\) poiché, non fosse così, per \(k>1\) potremmo semplicemente invertire i ruoli di \(x_h\) e \(x_k'\) nella fattorizzazione.
          Se \(k=1\) invece, si arriva a contraddizione poiché avremmo che \(x_1\ldots x_h = x_1'\), il che implica ovviamente che \(\abs{x_h} \leq \abs{x_1'}\).
  \end{enumerate}
...
\begin{observation}{}
  \begin{itemize}
    ...
    \item \(X \text{codice} \iff \forall n \geq 1, L_n \cap X = \emptyset\),
      ...
      La dimostrazione è analoga a quella del \Namedref{thm:sardinas-patterson}, utilizzando il corrispondente lemma per gli insiemi resto sinistri.
  \end{itemize}
\end{observation}
...
\begin{example}{}
  ...
  Poiché \(b \not\in X_2\), per il \Namedref{thm:sardinas-patterson}, \(X_2\) è codice.
  ...
  Anche in questo caso, \(X_4\) è codice per il \Namedref{thm:sardinas-patterson}.
  ...
  Dunque, essendo che \(a \in X'\), per il \Namedref{thm:sardinas-patterson}, \(X'\) non è codice.
\end{example}

Da questi risultati, si potrebbe evincere che il \Namedref{thm:sardinas-patterson} fornisca una procedura di decisione per stabilire se un insieme \(X \subseteq A^+\) \textbf{finito} sia codice o meno.
...
Di conseguenza, \(Suff(X)\) ha al più \(2^{KL+1}\) sottoinsiemi\footnote{\(\#(\mathcal{P}(S)) = 2^{\#S}\) è un risultato noto in combinatoria}, per cui \(\exists n \leq 2^{KL+1} +1 \st R_n = R_m, m < n\), e conseguentemente \(R_{n+i} = R_{m+i}, \forall i \geq 0\), ovvero la successione degli insiemi resto si stabilizza entro un numero finito di passi, esponenziale in \(K\) e \(L\).
...
\begin{proof}
  Per questa dimostrazione, sarà sufficiente dimostrare esclusivamente la direzione \(\impliedby\) della doppia implicazione, poiché l'altra direzione è ovvia per il \Namedref{thm:sardinas-patterson}.
  Procediamo dunque per assurdo, supponendo che \(X\) non sia codice. Allora per il \Namedref{thm:sardinas-patterson} \(\exists m > KL + 1 \st R_m \cap X \neq \emptyset\).
  Scegliamo, senza perdita di generalità, tale \(m\) come il minimo intero che soddisfa questa proprietà.
  Sia dunque \(r_m \in R_m \cap X\). Per definizione di insieme resto, esistono 
  \[x,x',x_2,\ldots,x_m \in X, r_1\in R_1,\ldots,r_{m-1} \in R_{m-1}\]
  tali che:
  \begin{itemize}
    \item \(xr_1=x'\) 
...
Questi esempi servono a mostrare come in molti casi questo nuovo limite sia comunque più grande del necessario, ma in ogni caso fornisce una procedura di decisione efficiente per stabilire se un insieme finito sia codice o meno.
...
\begin{corollary}[namedlabel={cor:kraft-mcmillan_inequality}{Disuguaglianza di Kraft-McMillan}]{Disuguaglianza di Kraft-McMillan}
...
\begin{definition}[namedlabel={def:structure_function}{Funzione di struttura di un insieme}]{Funzione di struttura di un insieme}
...
\begin{corollary}[namedlabel={cor:kraft-mcmillan_inequality_alt}{Disuguaglianza di Kraft-McMillan alt.}]{Disuguaglianza di Kraft-McMillan alt.}
  Dato \(X \subseteq A^+\), con \(\# A = d\), si ha:
...
\begin{proof}
  ...
  Dove entrambi i membri appartengono a \(X^{h+k} = X  X^{h+k-1}\), ma ammettono due fattorizzazioni distinte, dunque \(X \times X^{h+k-1}\) è ambiguo.
\end{proof}
...
\begin{proof}
  ...
  \begin{description}
    ...
    Formalmente:
    \[\mu(X)<\infty \land \mu(X^n) = {\mu(X)}^n, \forall n \geq 1 \implies \mu(X^n) = {\mu(X)}^n < \infty, \forall n \geq 1\]
    Dunque si ha che
  ...
  \end{description}
\end{proof}
Definite queste proprietà delle distribuzioni, e il loro rapporto coi codici e i prodotti non ambigui, possiamo finalmente andare a dimostrare il \Nameref{thm:kraft-mcmillan}, e quindi il\Namedref{cor:kraft-mcmillan_inequality}.

\begin{theorem}[namedlabel={thm:kraft-mcmillan}{Kraft-McMillan}]{Kraft-McMillan}
...
\begin{proof}
  ...
  Ovviamente, ogni \(X_k\) è finito.

  Inoltre si ha che, \(\forall k \geq 1\)
  \[X_{k+1}\setminus X_k = X \cap A^{k+1} \text{ e}\]
  \[X_k = X_1 \cup (X_2 \setminus X_1) \cup \ldots \cup (X_k \setminus X_{k-1}) = X_1 \cup \bigcup_{i=2}^{k} (X_i \setminus X_{i-1})\]
  ...
  Infine, si ha che \(X = X_1 \cup \bigcup_{i=2}^{\infty} (X_i \setminus X_{i-1})\), unioni disgiunte.
...
\begin{proof}[Dimostrazione \Namedref{cor:kraft-mcmillan_inequality}]
  ...
  \end{equation*}
  Applicando ora il \Namedref{thm:kraft-mcmillan}, si ha che:
...
Si potrebbe dunque pensare che il \Namedref{thm:kraft-mcmillan} possa invece esprimere una condizione necessaria e sufficiente per essere codice, ma ciò non è vero in generale.
...
\begin{example}{}
  ...
  Di conseguenza, tale insieme non può essere codice sia per il \Namedref{thm:kraft-mcmillan} che per il \Namedref{cor:kraft-mcmillan_inequality}
...
\begin{observation}{}
  L'ipotesi \emph{non denso} nel secondo punto del \Namedref{thm:schutz_maximality_completeness} è necessaria. Infatti il codice \(Z = \{a^{|u|+1}bu | u \in {\{a,b\}}^*\}\) è un codice denso (e quindi completo), ma non massimale.
...
\begin{proof}
  ...
  Dunque \(wyw \in Pref(w)\cdot X^* \cdot Suff(w)\), poiché il prefisso di \(wyw\) fino all'inizio di \(x_i\) è un prefisso di \(w\), il suffisso di \(wyw\) che parte dalla fine di \(x_j\) è un suffisso di \(w\), e la parte centrale è ottenuta concatenando parole di \(X\).
  ...
  Per far si che la serie geometrica diverga, è necessario che la sua ragione (in questo caso \(\mu(X)\)) sia maggiore o uguale a \(1\).
...
A questo punto siamo in grado di dimostrare il \Namedref{thm:schutz_maximality_completeness}.
\begin{proof}[Dimostrazione \Namedref{thm:schutz_maximality_completeness}]
  \begin{enumerate}
    \item Per dimostrare che se \(X\) è massimale allora è completo, procediamo per contrapposizione, ovvero assumendo che \(X\) non sia completo e mostrando che in tal caso non può essere massimale.
      ...
      Supponiamo dunque per assurdo che \textbf{non} lo sia.
    ...
    \item In questo caso, sappiamo per ipotesi che \(X\) è completo e non denso.
      ...
      D'altra parte, dal \Namedref{thm:kraft-mcmillan}, sappiamo che \(\mu(X) \leq 1\), poiché \(X\) è codice.
...
\end{corollary}
\begin{proof}
  Si ha che \(1 \iff 2\) per il punto \(1\) del \Namedref{thm:schutz_maximality_completeness}, \(2 \implies 3\) viene dalla dimostrazione del punto \(2\) del \Namedref{thm:schutz_maximality_completeness} per via della massimalità, \(3 \implies 4\) è banale e \(4 \implies 1\) è stato più volte dimostrato precedentemente.
\end{proof}
...
Vediamo dunque la dimostrazione dell'esistenza di un completamento per ogni codice, citata in precedenza.
\begin{proposition}{}
