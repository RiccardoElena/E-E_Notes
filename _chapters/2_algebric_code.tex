...
\begin{corollary}[Disuguaglianza di Kraft-McMillan]\label{cor:kraft-mcmillan_inequality}
...
\begin{corollary}[Disuguaglianza di Kraft-McMillan alt.]\label{cor:kraft-mcmillan_inequality_alt}
  Dato \(X \subseteq A^+\), con \(\# A = d\), si ha:
...
\begin{theorem}[Kraft-McMillan]\label{thm:kraft-mcmillan}
...
\begin{theorem}[Schützenberger su massimalità e completezza dei codici]\label{thm:schutz_maximality_completeness}
...
\begin{lemma}[Marcus-Schützenberger]
...
\begin{proof}[Dimostrazione di~\ref{thm:schutz_maximality_completeness}]
  \begin{enumerate}
    \item Per dimostrare che se \(X\) è massimale allora è completo, procediamo per contrapposizione, ovvero assumendo che \(X\) non sia completo e mostrando che in tal caso non può essere massimale.
      ...
      \todo{Potremmo considerare di trattare questa mini dim come lemma apparte?}
      ...
      Queste due occorrenze di \(y\) devono avere una sovrapposizione non vuota, poiché altrimenti si potrebbe completare \(y\) in \(X^*\) prendendo la parte tra la fine di \(y_i\) e l'inizio di \(y_j'\), contraddicendo l'ipotesi.
      Se esiste una sovrapposizione non vuota e non propria tra le due occorrenze di \(y\), allora \(y\) ammette bordi, poiché tale sovrapposizione è sia prefisso che suffisso di \(y\), contraddicendo l'ipotesi.
      Dunque l'unica possibilità rimasta è che le due occorrenze di \(y\) abbiano sovrapposizione propria, ovvero siano allineate nelle due fattorizzazioni.
      Essendo che \(y_1\neq y_1'\), necessariamente \(i\) e \(j\) non possono essere entrambi \(1\), e per non violare la minimalità di \(i\) e \(j\), devono essere entrambi maggiori di \(1\).
      Ma allora, \(y_1\ldots y_{i-1} = y_1' \ldots y_j'\). Tale uguaglianza è ottenuta concatenando parole di \(X\) (essendo che \(y\) compare solo a partire da \(i\) e \(j\)), dunque si ha che \(X\) non è codice, contraddicendo l'ipotesi.
      Abbiamo dunque raggiunto una contraddizione in ogni caso, per cui l'ipotesi che \(Y\) non sia codice è falsa.
      Dunque \(Y\) è codice, e quindi \(X\) non è massimale.
    \item In questo caso, sappiamo per ipotesi che \(X\) è completo e non denso.
      Dunque presa \(\mu\) distribuzione positiva su \(A\), dal Lemma precedente sappiamo che \(\mu(X) \geq 1\).
      D'altra parte, dal Teorema di Kraft-McMillan~\ref{thm:kraft-mcmillan}, sappiamo che \(\mu(X) \leq 1\), poiché \(X\) è codice.
      Dunque, si ha che \(\mu(X) = 1\). Come visto in precedenza, ciò implica che \(X\) è massimale.
  \end{enumerate}
\end{proof}


\todo{Possibile versione del lemma estrapolare dal teorema precedente}
