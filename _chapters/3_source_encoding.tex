...
\begin{note}{}
  ...
  In tal caso il termine della somma corrispondente a tale \(s\) non saredefinito. 
...
In oltre, nel \Namedref{thm:prefix_suffix_code} abbiamo visto che a un insieme prefisso (o suffisso) per essere codice è sufficiente che non sia \(\set{\varepsilon}\), che è ovviamente sia prefisso che suffisso ma non codice.
...
\begin{definition}{Ritardo di decifrazione}
  ...
  \begin{equation*}
    DD(k) = \set{C\subseteq A^* }[\forall x,x' \in X, (\set{x}X^k A^+ \cap \set{x'}X^* \neq \emptyset) \implies x = x']
  \end{equation*}
