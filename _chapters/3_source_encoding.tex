...
\begin{definition}{Sorgente (discreta e a memoria zero)}
  Chiamiamo \keyword{Sorgente discreta e a memoria zero} una variabile aleatoria discreta, identificabile come una coppia \(S = (\SCal,p)\), con \(\SCal\) alfabeto sorgente finito, ovvero l'insieme dei simboli che la sorgente può emettere,
...
\begin{definition}{Codifica}
  ...
  Diremo inoltre che un codice è \keyword{adattabile} alla sorgente \(S\) se \(\# X = \# \SCal\). In questo caso è possibile definire una codifica biettiva tra i simboli della sorgente e le parole del codice.
  In particolare, tale codice sarà \keyword{adattato} a una sorgente \(S\) mediante \(\varphi\), se \(\varphi\) induce una biiezione tra \(\SCal\) e \(X\), \(\varphi_{|_X}: \SCal \leftrightarrow X\).
\end{definition}
...
\begin{proposition}{}
  ...
  Allora \(c(X) = c(X,\varphi)\) se e solo se
    \[\forall s,s' \in \SCal, p(s) < p(s') \implies \abs{\varphi(s)} \geq \abs{\varphi(s')} \]
\end{proposition}
...
\begin{proof}[Dimostrazione di~\ref{eq:lower_bound_entropy}]
  ...
  Dunque \(H(S) = \sum_{s \in \SCal} p(s) \log\left(\frac{1}{p(s)}\right) = \EV{\log\left(\frac{1}{p(S)}\right)} \geq 0\), essendo somma pesata (o media) di quantità non negative.
...
\begin{proof}[Dimostrazione di~\ref{eq:upper_bound_entropy}]
  ...
  \begin{note}{}
    È importante notare che il lato destro della disuguaglianza può divergere, poiché essendo le distribuzioni diverse non è possibile definire separatamente il caso dello \(0\), poiché le due distribuzioni potrebbero essere nulle in punti diversi.
  \end{note}
...
\begin{theorem}[namedlabel={thm:shannon}{Shannon}]{Shannon}
...
\begin{proof}
  ...
  Essendo \(X\) un codice, per il \Namedref{cor:kraft-mcmillan_inequality} si ha che \(\pi(X) \leq 1\).
  Dunque:
...
È però sempre possibile, per ogni sorgente e alfabeto di codice (con almeno \(2\) lettere) avere un codice ottimale, e lo si può scegliere prefisso.
...
\emph{codice} & \emph{codice univocamente decifrabile} (eng.\ \emph{univocally decifrable code}) \\
...
Inoltre, nel \Namedref{thm:prefix_suffix_code} abbiamo visto che a un insieme prefisso (o suffisso) per essere codice è sufficiente che non sia \(\set{\varepsilon}\), che è ovviamente sia prefisso che suffisso ma non codice.
...
\begin{definition}[namedlabel={def:complete_tree}{Albero completo}]{Albero completo}
...
\begin{note}{}
  ...
  Nel contesto di questo corso, e di conseguenza in questi appunti, per albero completo si intenderà sempre quello della \Cref{def:complete_tree}.
\end{note}
...
\begin{proof}
  Si ha che \(1 \implies 2\) da \Namedref{thm:prefix_code_properties}, e \(2 \implies 3\) è ovvio, \(3,4,5,6\) sono equivalenti da \Cref{cor:schutz_maximality_completeness} e \(6 \implies 1\) è ovvio.
\end{proof}
...
\begin{theorem}[namedlabel={thm:kraft}{Kraft}]{Kraft}
  Sia \(d \geq 1\) un intero e sia \(f: \N \to \N\) tale che
  \[f(0) = 0 \text { e }\sum_{n=0}^\infty f(n)d^{-n} \leq 1\]
  
  Allora esiste \(X\) codice prefisso su alfabeto \(A\) con \(\#A = d\) tale che \(f_X = f\)
\end{theorem}

\begin{observation}{}
  In particolare, se \(X\) è un codice non contiene \(\varepsilon\), e dunque \(f_X(0) = 0\).
  Inoltre, per la disuguaglianza di Kraft---McMillan, si ha che \(\sum_{n=0}^\infty f_X(n)d^{-n} = \pi(X) \leq 1\).
...
\begin{proof}
  ...
  \[\nu(k)=d^k-\sum_{n=0}^{k-1}f(n)d^{k-n} = d(d^{k-1}-\sum_{n=0}^{k-1}f(n)d^{k-n-1}) = \]
  \[= d(d^{k-1}-\sum_{n=0}^{k-2}f(n)d^{k-n-1}-f(k-1)d^{0}) = d(\nu(k-1)-f(k-1))\]

  Si ha ovviamente per costruzione che \(f(n)\leq\nu(n), \forall n \in \N\)
  In particolare si ha che \(f(1) \leq \nu(1) = d\).
  Considerando dunque l'albero \(d\)-ario generico possiamo scegliere \(f(1)\) nodi al primo livello per potarne i sottoalberi corrispondenti rendendoli foglie.
  Ognuno dei rimanenti \(d - f(1)\) nodi al primo livello ha \(d\) figli al secondo, e \(d(d-f(1))=d(\nu(1)- f(1)) = \nu(2)\).
  Dunque possiamo scegliere \(f(2) \leq \nu(2)\) nodi al secondo livello e potarne i sottoalberi corrispondenti rendendoli foglie.
...
\begin{proof}
  Segue direttamente dal \Namedref{cor:kraft-mcmillan_inequality_alt} e dal \Namedref{thm:kraft}.
\end{proof}
...
Dal \Namedref{thm:shannon}, se \(\#A = d \geq 2\) e \(\varphi: \SCal \leftrightarrow X\)
...
\begin{proof}
  ...
  Per il \Namedref{thm:shannon}, abbiamo che 
  ...
  Da cui \(f(0) = 0\). Sono dunque valide le ipotesi del \Namedref{thm:kraft}, e dunque esiste \(X\) codice prefisso su \(A\) tale che \(f_X = f\), e in particolare si ha che \(\lvert\; \rvert \circ \varphi = r\), ovvero la funzione lunghezza delle parole di \(X\) coincide con \(r\).
...
\begin{proof}
  Assumiamo \(X_i = \set{x_1, \ldots,x_{i-1},x_i}\) e \(X_{i+1} = \set{x_1,\ldots,x_{i-1},x_{i}a, x_{i}b}\), con queste ultime associate a simboli di probabilità \(q_1\geq q_2 \geq \ldots \geq q_{i-1} \geq q_i \geq q_{i+1}\) e \(x_i\) associata a un simbolo di \(S_i\) di probabilità \(q_i + q_{i+1}\).
  
  Supponiamo per assurdo che \(X_{i+1}\) \textbf{non} sia ottimale per \(S_{i+1}\) e chiamiamo \(Y_{i+1} \) il prefisso ottimale (binario) per \(S_{i+1}\).
  Poiché \(Y_{i+1}\) è ottimale, è massimale e dunque \(T_{Y_{i+1}}\) è completo.

  Dunque \(\exists y_{i/i+1} \in Pref(Y_{i+1}),y_i, y_{i+1} \in Y_{i+1}\) tali che \(y_i = y_{i/i+1}a\) e \(y_{i+1} = y_{i/i+1}b\) con \(a,b \in A, a \neq b\).

  Nel calcolo di \(c(Y_{i+1})\) tali parole dovranno essere associate a simboli di probabilità \(q_i\) e \(q_{i+1}\).
  Sia allora \(Y_{i+1} = \set{y_1,\ldots,y_{i-1},y_i,y_{i+1}} \text{ con } y_i=y_{i/i+1}a,y_{i+1} = y_{i/i+1}b\) e definiamo \(Y_i = \set{y_1,\ldots,y_{i-1}, y_{i/i+1}}\).

  Si ha \(c(Y_i) \leq \sum_{j=1}^{i-1} q_j\abs{y_j} + (q_i+q_{i+1})\abs{y_{i/i+1}}\).

  Possiamo inoltre osservare che \(\abs{y_{i/i+1}} = \abs{y_{i}}-1 = \abs{y_{i+1}}-1\).
  Dunque
  \[c(Y_i) \leq \sum_{j=1}^{i-1} q_j\abs{y_j} + q_i(\abs{y_{i}}-1) + q_{i+1}(\abs{y_{i+1}}-1) = \]
  \[=\sum_{j=1}^{i+1} q_j\abs{y_j} - (q_i + q_{i+1}) = c(Y_{i+1}) - (q_i + q_{i+1}) < c(X_{i+1}) - (q_i + q_{i+1})\]

  Ma il codice \(X_i\) è ottenibile da \(X_{i+1}\) nello stesso modo in cui \(Y_i\) è ottenuto da \(Y_{i+1}\).
  Dunque \(c(Y_i) \leq c(Y_{i+1}) - (q_i + q_{i+1}) < c(X_{i+1}) - (q_i + q_{i+1}) = \sum_{j=1}^{i+1} q_j\abs{x_j} - (q_i + q_{i+1})\).
  Avendo detto che \(X_i\) è ottimale per \(S_i\),  ottenuto associando a \(x_i\) un simbolo di probabilità \(q_i + q_{i+1}\), si ha che
  \[c(Y_i) \leq c(Y_{i+1}) - (q_i + q_{i+1}) < c(X_{i+1}) - (q_i + q_{i+1}) = \sum_{j=1}^{i+1} q_j\abs{x_j} - (q_i + q_{i+1}) = \]
  \[=\sum_{j=1}^{i-1} q_j\abs{x_j} + q_i(\abs{x_i a}-1) + q_{i+1}(\abs{x_{i+1}b}-1) = \sum_{j=1}^{i-1} q_j\abs{x_j} + (q_i+q_{i+1})\abs{x_{i}} = c(X_i)\]
  Abbiamo dunque trovato che \(c(Y_i)<c(X_i)\) che contraddice l'ipotesi che \(X_i\) sia ottimale per \(S_i\), portando all'assurdo
\end{proof}
...
Nel caso generale, se \(\# \SCal = n \not\equiv_{d-1} 1\), significa che a uno step della costruzione dell'albero non tutti i nodi avranno \(d\) figli.
...
meno probabili, invece di \(d\). In questo modo si ottiene un albero a cui sono state rimosse \(r\) foglie e aggiunte una, dunque si avrà che il nuovo albero avrà \(n - r + 1\) foglie, che soddisfa
\[n - r + 1 \equiv_{d-1} 1\]
...
Prendiamo ad esempio un codice con ritardo di decifrazione \(2\).
Questo intuitivamente ci dice che, qualsiasi parola del codice, dopo aver ricevuto al più \(2\) parole aggiuntive può essere decifrata.
...
Inoltre, il codice Morse è sincronizzante, poiché ogni parola di codice finisce con uno spazio (pausa più lunga), e quindi ogni coppia di parole è sincronizzante.
