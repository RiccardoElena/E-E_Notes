...
\begin{example}{}
  Il codice \(X = \set{ab,ba,bb}\) non è circolare. Considerando infatti la parola \(abab\), posso ottenerla nei seguenti modi:
  \todo{IMMAGINE}
  Inoltre, qualsiasi codice che contenga una parola che è potenza di una lettera non può essere circolare.
  \todo{IMMAGINE}
  Questo si può banalmente vedere considerando il caso in cui \(a^n \in X\) con \(a \in A\). Si ha necessariamente che \(a^{2n} = a^n a^n \in X^*\) e dunque la fattorizzazione non è circolarmente univoca.
\end{example}
