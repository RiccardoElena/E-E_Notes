...
\begin{itemize}
  ...
  \item \(\mathcal{F} \subseteq \mathcal{P}(\Omega)\) è una \(\sigma\)-algebra su \(\Omega\), cioè un insieme di sottoinsiemi di \(\Omega\) che contiene \(\Omega\) stesso, chiuso rispetto al complemento e alle unioni numerabili;
  \item \(\Prob: \mathcal{F} \to [0,1]\) è una misura di probabilità, cioè una funzione che rispetta gli assiomi di Kolmogorov:
    \begin{enumerate}
      \item \(\Prob[A] \geq 0, \forall A \in \mathcal{F}\)
      \item \(\Prob[\Omega] = 1, \Prob[\emptyset] = 0\)
      \item \(\Prob\) è \(\sigma\)-additiva, ovvero \(\Prob[\bigcup_{i=1}^\infty E_i] = \sum_{i=1}^{\infty} \Prob[E_i]\), se \(\forall i,j \in \N, i \neq j \implies E_i \cap E_j = \emptyset\)
    \end{enumerate}
\end{itemize}
...
\begin{definition}{Entropia di una variabile aleatoria}
  ...
  \[H(X) = -\sum_{x \in \mathcal{X}} p(x) \log( p(x)) = \EV{\log\left(\frac{1}{p(X)}\right)}\]
\end{definition}
...
Di conseguenza, data una probabilità congiunta \(p(x,y)\), è sempre possibile ricavare le distribuzioni marginali di \(X\) e \(Y\), ma non il contrario.}
...
Possiamo dunque andare a generalizzare questi risultati a più variabili aleatorie.
...
In particolare, per \(f = id\) si ha che \(I(X;X) = H(X)\), giustificando il significato intuitivo dato precedentemente all'entropia come misura dell'autoinformazione di una variabile aleatoria.
...
\end{proposition}
\begin{proof}
  Essendo che in generale \(I(X;Y)= H(X) - H(X|Y) \geq 0\), si ha che \(H(X) \geq H(X|Y) \), ovvero il condizionamento non aumenta l'entropia.
...
\begin{proof}
  ...
  Essendo che \(p(x,z|y)\geq 0\), \(\forall x \in \mathcal{X}\), si ha che \(\sum_{x\in\mathcal{X}}p(x,z|y) = 0 \iff p(x,z|y)=0 \; \forall x\), e dunque \(0=p(x,z|y) = p(x|y)p(z|y) = p(x|y)\cdot 0\).
...
\begin{proposition}[namedlabel={prop:dpi}{Disuguaglianza di Data Processing (DPI)}]{Disuguaglianza di Data Processing (DPI)}
...
\begin{definition}{Catene irriducibili e aperiodiche}
  ...
  \[\forall i,h \in \mathcal{X}, \exists n \geq 1 \st \Prob[X_{n+1} = h|X_1 = i] > 0\]

  Una catena \(\underline{X}\) si dice \keyword{aperiodica} se
  \[\forall x \in \mathcal{X}, \gcd \set{n\in\N}[\Prob[X_{n+1} = x|X_1=x]>0] = 1\]
\end{definition}

Le catene di Markov tempo invarianti possono essere modellate come grafi orientati pesati, con i nodi che rappresentano gli stati (elementi di \(\mathcal{X}\)) e gli archi che rappresentano le transizioni (con peso dato dalla probabilità di transizione).\footnote{È possibile solo per le catene tempo invarianti poiché altrimenti la matrice di transizione non unica non permetterebbe di assegnare agli archi pesi unici}
In tal senso, si ha che una catena è irriducibile se e solo se il grafo è fortemente connesso, e che una catena è aperiodica se e solo se, per ogni nodo \(x\), le lunghezze dei cicli che partono e finiscono in \(x\) sono coprime.
In altre parole, una catena è aperiodica se non ha cicli (incompatibile con l'irriducibilità) o se, per ogni nodo \(x\), esistono almeno due cicli semplici in \(x\) con lunghezze coprime.

Si dimostra che una catena invariante nel tempo, aperiodica e irriducibile ha un'unica distribuzione stazionaria e che indipendentemente dalla distribuzione iniziale \(p_1\), se \(\mu\) è tale distribuzione stazionaria, si ha che \[\lim_{n\to\infty} p_n = \mu\]
...
\begin{proof}
  ...
  Inoltre, dalla regola di catena,
  \[\frac{1}{n} H(X_1, \ldots, X_n) = \frac{1}{n}\left(H(X_1) +\sum_{i=2}^n H(X_i|X_{i-1}, \ldots, X_1)\right)\]
  cioè \(\frac{1}{n} H(X_1, \ldots, X_n)\) è la media aritmetica parziale dei primi \(n\) termini della successione \(a_n\).

  Per il teorema della media di Cesàro\footnote{Il teorema della media di Cesàro afferma che se una successione \(a_n\) ammette limite, allora il limite delle sue medie aritmetiche parziali converge allo stesso limite. In termini più formali, dato \(\sigma_n = \frac{1}{n}\sum_{i=1}^n a_i\), si ha \[\lim_{n\to\infty} \sigma_n = \lim_{n\to\infty} a_n\]}, si ricava \(H(\underline{X}) = H'(\underline{X})\).
\end{proof}
