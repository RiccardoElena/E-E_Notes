...
\begin{theorem}[namedlabel={thm:fano}{Disuguaglianza di Fano}]{Disuguaglianza di Fano}
...
\begin{proof}
  ...
  Infine, abbiamo, per definizione di entropia condizionata, che \(H(X|E,\hat{X})\) si può scrivere come
  ...
  L'ipotesi che \(X \to Y \to \hat{X}\) è necessaria solo per la prima disuguaglianza \(H(X|Y) \leq H(X|\hat{X})\), che segue in modo diretto dalla \Namedref{prop:dpi}.
\end{proof}
\begin{observation}{}
  ...
  Sia \(\mathcal{X} = \set{1,\ldots,m}\) e sia \(p_{\hat{X}}(1) = 1\), ovvero ogni altro valore di \(\mathcal{X}\) abbia probabilità nulla nella distribuzione di \(\hat{X}\).
...
\begin{definition}{Estensione \(n\)-esima di un DMC}
  ...
  ovvero il \(k\)-esimo output dipende solo dal \(k\)-esimo input.
  
  Diciamo che il canale è utilizzato \keyword{senza feedback} se, per \(1\leq k \leq n\),
  \[p(x_k|x_1,\ldots,x_{k-1},y_1,\ldots,y_{k-1}) = p(x_k|x_1,\ldots,x_{k-1})\]
  ovvero anche il \(k\)-esimo input dipende solo dagli input precedenti, e non anche dagli output.
\end{definition}
...
\begin{definition}{Codifica di canale a blocchi}
  Dato un DMC \((\mathcal{X}, \mathcal{Y}, p(y|x))\), e dati \(M>1\) e \(n\geq 1\), una \keyword{\((M,n)\)\nobreakdash-codifica} dell'estensione \(n\)-esima del DMC, è una terna \((\mathcal{W},f,g)\) con
  \begin{itemize}
...
\begin{definition}{Tasso di trasmissione}
  Data una \((M,n)\)\nobreakdash-codifica \((\mathcal{W},f,g)\), il \keyword{tasso di trasmissione \(R\)} è la quantità
  \[R = \frac{\log M}{n}\] 
  che rappresenta la quantità di informazione trasmessa per simbolo del canale.
  
  Fissato \(R>0\), si ha dunque che \(M = 2^{nR}\).
\end{definition}

\begin{definition}{Tasso raggiungibile}
  Diremo che un tasso di trasmissione \(R\) è \keyword{raggiungibile} se esiste una successione di \((\lfloor 2^{nR} \rfloor, n)\)\nobreakdash-codifiche tali che \(\lim_{n\to\infty} \lambda^{(n)} = 0\).
\end{definition}

Si ha dunque che una \((M,n)\)\nobreakdash-codifica porta a una struttura del tipo
\begin{equation}
  W \xrightarrow{f} X^n \xrightarrow{p(y^n|x^n)} Y^n \xrightarrow{g} \hat{W}
\end{equation}
con \(p(y^n|x^n) = p(y_1,\ldots,y_n|x_1,\ldots,x_n)\)

Dato un elemento di \(\mathcal{W}\), possiamo scrivere la codifica \(f\) come \(f(i) = x_1(i)\ldots x_n(i) = x^n(i)\) e quindi
...
\begin{definition}{Probabilità media (aritmetica) di errore}
  La \keyword{probabilità media (aritmetica) di errore} di una \((M,n)\)\nobreakdash-codifica \((\mathcal{W},f,g)\) è
...
\begin{theorem}[namedlabel={{thm:shannon_2}}{Shannon}]{Shannon}
...
\begin{proof}[Dimostrazione \Namedref{thm:shannon_2}]
  ...
  Sia allora \(R\) un tasso di trasmissione raggiungibile, e consideriamo la successione di \(( 2^{nR},n)\)\nobreakdash-codifiche\footnote{Dalla definizione sarebbe necessario utilizzare la funzione \(\lfloor\rfloor\), ma ciò andrebbe solo a complicare i conti, dunque si assumeranno \(2^{nR}\) interi per semplicità} tale che \(\lim_{n\to\infty} \lambda^{(n)} = 0\).
