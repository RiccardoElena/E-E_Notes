...
\begin{definition}{Funzioni di Decriptazione}
  Dato un crittosistema, un insieme di funzioni \(\mathcal{D}_{\mathcal{K}} = \set{D_l}[l\in\mathcal{K}]\subseteq {(\mathcal{M}^{\mathcal{C}})}^{\mathcal{K}}\) è detto di \keyword{decrittazione} se
...
\begin{example}{}
  ...
  Fissato un ordine sull'alfabeto \(\SCal\) e scelta una lettera di posizione \(k\), ogni lettera di posizione \(a\) viene cifrata nella lettera di posizione \(b \equiv_{\# \SCal} a+k,  0\leq b\leq \# \SCal -1\).
...
In tali sistemi si usa come chiave di criptazione una permutazione \(\sigma\) di \(\SCal\),  (\(k = \sigma, l = \sigma^{-1}\)).
...
Per criptare il messaggio, si calcola per ogni \(i \in \set{1,\ldots, h}\) il blocco cifrato \(c_i^T = K m_i^T\), ottenendo così il messaggio cifrato tramite la concatenazione \(c = E_{k}(m) = c_1\ldots c_h\).
...
Chiaramente questo richiede che i crittosistemi siano tali che sia facile decifrare conoscendo la chiave privata, ma sia computazionalmente difficile decifrare od ottenere la chiave privata conoscendo solo la chiave pubblica.
...
\[\phi(p_1^{\alpha_1}\cdot\ldots\cdot p_t^{\alpha_t}) = p_1^{\alpha_1-1}\cdot\ldots \cdot p_t^{\alpha_t-1}\cdot(p_1 - 1)\cdot\ldots\cdot(p_t - 1) = \prod_{i=1}^{t} (p_i-1)p_i^{\alpha_i-1}\]
...
\begin{itemize}
  \item Bob sceglie due numeri primi grandi \(p,q\) e calcola \(n = p q\) e \(\phi(n) = (p-1)(q-1)\).
  \item Bob sceglie un intero \(e\) coprimo con \(\phi(n)\) e calcola il suo inverso moltiplicativo \(d \equiv_{\phi(n)} e^{-1}\).
  \item Bob pubblica la chiave pubblica \((n,e)\) e tiene segreta la chiave privata \((n,d)\).
\end{itemize}

Prendendo dunque un \(m\) (blocco di lunghezza \(\lceil \log n \rceil\)) si ha che la sua cifratura sarà \(E_{(n,e)} (m) = m^e \mod n\) e la decifratura \(D_{(n,d)} (c) = c^d \mod n\).

Prima di addentrarci nella dimostrazione di correttezza di RSA, qualche osservazione pratica:
\begin{itemize}
  \item il \(\gcd\) e l'inverso moltiplicativo possono essere calcolati con l'algoritmo di Euclide in tempo polinomiale
  \begin{example}{}
...
\begin{theorem}{Fermat-Euler}
  Siano \(n,m \in \N'\) con \(\gcd(m,n) = 1\).
  Allora \(m^{\phi(n)} \equiv_{n} 1\).
\end{theorem}

\begin{theorem}{Piccolo Teorema di Fermat}
  Sia \(p\) un numero primo e \(a \in \N'\) con \(\gcd(a,p) = 1\).
  Allora \(a^{p-1} \equiv_{p} 1\).
\end{theorem}

\begin{proposition}{Correttezza di RSA}
  Dato \(n = q p\), con \(p,q\) primi distinti, \(e \in N\) coprimo con \(\phi(n) = (p-1)(q-1)\) e \(d \equiv_{\phi(n)} e^{-1}\), allora
  \[\forall m \in \set{0,\ldots,n-1}, {(m^e)}^d \equiv_{n} m \]
\end{proposition}
...
\begin{proof}
  ...
  \begin{enumerate}
    ...
    \item Consideriamo dunque \(m\) coprimo con \(n\), ovvero \(\gcd(m,n) = 1\).
      ...
      Quindi \(m^{ed} = m^{1 + k\phi(n)} = m{(m^{\phi(n)})}^k\).
    ...
    \item Infine, consideriamo \(m\) multiplo di \(p\) e \(\gcd(m,q) = 1\).
      ...
      Inoltre \(m^{q-1} \equiv_q 1\) Dal piccolo teorema di Fermat. Dunque
